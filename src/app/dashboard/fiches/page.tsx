...
export default function FichesPage() {
  const [fiches, setFiches] = useState<Document[]>([]);
  ...
  const [editingFiche, setEditingFiche] = useState<Document | null>(null);
  const [selectedFiche, setSelectedFiche] = useState<Document | null>(null);
  ...
  const loadFiches = () => {
    ...
    setFiches(allDocs.filter(d => d.type === 'fiche'));
  };
  ...
  const handleOpenEdit = (fiche: Document, e: React.MouseEvent) => {
    e.stopPropagation();
  ...
  const handleSave = (payload: any) => {
    ...
    if (editingFiche) {
      updateDocument(editingFiche.id, docPayload);
    } else {
  ...
  const handleOpenViewer = (fiche: Document) => {
  ...
  const filteredFiches = fiches.filter(f => {
    const matchesSearch = 
      (f.eleveNom || '').toLowerCase().includes(search.toLowerCase()) ||
      f.ecole.toLowerCase().includes(search.toLowerCase()) ||
      (f.metierOption || '').toLowerCase().includes(search.toLowerCase()) ||
      f.classe.toLowerCase().includes(search.toLowerCase());
    
    const matchesProvince = selectedProvince === 'Toutes' || f.province === selectedProvince;
    const matchesAnnee = selectedAnnee === 'Toutes' || f.anneeScolaire === selectedAnnee;
    const matchesEcole = selectedEcole === 'Toutes' || f.ecole === selectedEcole;

    return matchesSearch && matchesProvince && matchesAnnee && matchesEcole;
  });

  const provinces = Array.from(new Set(fiches.map(f => f.province)));
  const annees = Array.from(new Set(fiches.map(f => f.anneeScolaire)));
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-[var(--color-text-primary)] flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-[#12b886] to-[#38d9a9] flex items-center justify-center">
              <ClipboardList className="w-5 h-5 text-white" />
            </div>
            Gestion des Fiches
          </h1>
          <p className="text-sm text-[var(--color-text-secondary)] mt-1">
            Fiches administratives des élèves — données personnelles et scolaires
          </p>
        </div>
        <button 
                ...
                filteredFiches.map((f) => (
                  ...
                  >
                    ...
                    <td className="px-5 py-4 shrink-0">
                      <div 
                        onClick={(e) => { e.stopPropagation(); handleOpenViewer(f); }}
                    ...
                    <td className="px-5 py-4">
                      <p className="font-semibold text-[var(--color-text-primary)]">{f.eleveNom}</p>
                      <p className="text-xs text-[var(--color-text-muted)]">{f.metierOption || 'Enseignement Général'}</p>
                    </td>
                    <td className="px-5 py-4 text-[var(--color-text-secondary)]">{f.ecole}</td>
                    <td className="px-5 py-4 text-[var(--color-text-secondary)]">{f.province}</td>
                    <td className="px-5 py-4 text-[var(--color-text-secondary)]">{f.classe}</td>
                    <td className="px-5 py-4 text-[var(--color-text-muted)]">{f.anneeScolaire}</td>
                    <td className="px-5 py-4">
