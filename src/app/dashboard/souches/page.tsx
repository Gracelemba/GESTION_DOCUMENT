...
import { BookCopy, Plus, Search, Eye, Pencil, Trash2 } from 'lucide-react';
...
export default function SouchesPage() {
  const [souches, setSouches] = useState<Document[]>([]);
  ...
  const [editingSouche, setEditingSouche] = useState<Document | null>(null);
  const [selectedSouche, setSelectedSouche] = useState<Document | null>(null);
  ...
  const loadSouches = () => {
    ...
    setSouches(allDocs.filter(d => d.type === 'souche'));
  };

  const handleOpenAdd = () => {
    setEditingSouche(null);
    setIsModalOpen(true);
  };

  const handleOpenEdit = (souche: Document, e: React.MouseEvent) => {
    e.stopPropagation();
    setEditingSouche(souche);
    setIsModalOpen(true);
  };

  const handleSave = (payload: any) => {
    const docPayload = {
      type: 'souche' as const,
      ...payload,
    };

    if (editingSouche) {
      updateDocument(editingSouche.id, docPayload);
    } else {
  ...
  const handleDelete = (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (confirm('Voulez-vous vraiment supprimer cette souche ?')) {
      deleteDocument(id);
      loadSouches();
    }
  };

  const handleOpenViewer = (souche: Document) => {
    setSelectedSouche(souche);
    setIsViewerOpen(true);
  };

  // Search & Filter Logic
  const filteredSouches = souches.filter(s => {
    const matchesSearch = 
      (s.eleveNom || '').toLowerCase().includes(search.toLowerCase()) ||
      s.ecole.toLowerCase().includes(search.toLowerCase()) ||
      (s.metierOption || '').toLowerCase().includes(search.toLowerCase()) ||
      s.classe.toLowerCase().includes(search.toLowerCase());
    
    const matchesProvince = selectedProvince === 'Toutes' || s.province === selectedProvince;
    const matchesAnnee = selectedAnnee === 'Toutes' || s.anneeScolaire === selectedAnnee;
    const matchesEcole = selectedEcole === 'Toutes' || s.ecole === selectedEcole;

    return matchesSearch && matchesProvince && matchesAnnee && matchesEcole;
  });

  const provinces = Array.from(new Set(souches.map(s => s.province)));
  const annees = Array.from(new Set(souches.map(s => s.anneeScolaire)));
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[var(--color-text-secondary)] mt-1">
            Souches de délivrance de documents — numérotation et signatures
          </p>
        </div>
        <button 
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              placeholder="Rechercher par élève, option, classe..."
      ...
      <div className="bg-[var(--color-surface)] rounded-2xl border border-[var(--color-border-light)] overflow-hidden shadow-sm">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filteredSouches.length > 0 ? (
                filteredSouches.map((s) => (
                  <tr 
                    key={s.id} 
                  ...
                  >
                    ...
                    <td className="px-5 py-4 shrink-0">
                      <div 
                        onClick={(e) => { e.stopPropagation(); handleOpenViewer(s); }}
                    ...
                    <td className="px-5 py-4">
                      <p className="font-semibold text-[var(--color-text-primary)]">{s.eleveNom}</p>
                      <p className="text-xs text-[var(--color-text-muted)]">{s.metierOption || 'Enseignement Général'}</p>
                    </td>
                    <td className="px-5 py-4 text-[var(--color-text-secondary)]">{s.ecole}</td>
                    <td className="px-5 py-4 text-[var(--color-text-secondary)]">{s.province}</td>
                    <td className="px-5 py-4 text-[var(--color-text-secondary)]">{s.classe}</td>
                    <td className="px-5 py-4 text-[var(--color-text-muted)]">{s.anneeScolaire}</td>
                    <td className="px-5 py-4">
                ...
                <tr>
                  <td colSpan={8} className="px-5 py-8 text-center text-[var(--color-text-muted)]">
                    Aucune souche ne correspond à vos critères.
                  </td>
                </tr>
              )}
            </tbody>
